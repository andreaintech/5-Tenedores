...
export default function ChangePasswordForm(props) {
  ...
  const onSubmit = async () => {
    ...
    if (!formData.password || !formData.newPassword || !formData.repeatNewPassword) {
      ...
      errorsTemp = {
        newPassword: "La contraseña no puede ser menor a 6 caracteres",
        repeatNewPassword: "La contraseña no puede ser menor a 6 caracteres",
      };
    } else {
      setIsLoading(true);
      ...
      await reauthenticate(formData.password)
        .then(async () => {
          await firebase
            ...
            .catch(() => {
              errorsTemp = {
                other: "Error al modificar la contraseña",
              };
              setIsLoading(false);
            });
        })
